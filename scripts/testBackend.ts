...
const serviceAccountPath = join(__dirname, '..', 'starmenspark-1cc3f-firebase-adminsdk-fbsvc-da12b4023b.json');
...
async function testBackend() {
  ...
  try {
    ...
    if (usersSnapshot.size > 0) {
      ...
      console.log('   Run: npx ts-node scripts/makeAdmin.ts <email>\n');
    }
    ...
    if (productsSnapshot.size === 0) {
      console.log('\n⚠️  ACTION REQUIRED:');
      console.log('   1. Make user admin: npx ts-node scripts/makeAdmin.ts <email>');
      console.log('   2. Login to /admin and add products');
      console.log('   3. Or run: npx ts-node scripts/uploadProducts.ts\n');
    } else {
